...
gst-launch-1.0 -v rtpbin name=rtpbin \
     $ASOURCE ! $AENC ! rtpbin.send_rtp_sink_0  \
            rtpbin.send_rtp_src_0 ! udpsink port=6002 host=$DEST                      \
            rtpbin.send_rtcp_src_0 ! udpsink port=6003 host=$DEST sync=false async=false \
         udpsrc port=6007 ! rtpbin.recv_rtcp_sink_0